...
/**
Base Op for running command sequence benchmarks
*/
export class CommandSequenceBenchmarkOp extends BaseOp<BenchmarkResult>
{
  ...
  protected async performOp(): Promise<Outcome<BenchmarkResult>>
  {
    ...
    try
    {
      ...
      else
      {
        ...
        return {
          success: true, // Op itself succeeded, benchmark failed
          data: {
            name: this.benchmarkName,
            target: this.target,
            duration,
    ...
    catch (error: unknown)
    {
      ...
      return {
        success: false,
        error: `Benchmark Op failed: ${errorMessage}`,
      };
    }
  }
...
if (import.meta.main)
{
  const op = new CommandSequenceBenchmarkOp({
    ...
    description: 'Test the command sequence benchmark Op',
    commands: [
      {
        name: 'Create file',
        cmd: 'echo',
