...
import { InputTextOp, type InputValidation } from './InputTextOp.ts';
...
/**
Op for collecting multiple text inputs with back navigation support
...
*/
export class InputMultipleTextOp extends UIOp<MultipleTextResult | null>
{
  ...
  protected async performOp(): Promise<Outcome<MultipleTextResult | null>>
  {
    try
    {
      ...
      while (currentIndex < this.fields.length)
      {
        ...
        if (value === null)
        {
          if (currentIndex > 0)
          {
            ...
            const navOp = new SelectOp('What would you like to do?', choices, false);
            const navResult = await navOp.run();
...
if (import.meta.main)
{
  const example = new InputMultipleTextOp(
